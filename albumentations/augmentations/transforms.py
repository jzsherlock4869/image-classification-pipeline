...
class NormalizeMultispectral(ImageOnlyTransform):
    """subtract mean per channel and divide by std per channel. all values are of real amplitude
    ...
    def __init__(self, mean, std, always_apply=False, p=1.0):
        super(NormalizeMultispectral, self).__init__(always_apply, p)
        self.mean = mean
        self.std = std

    def apply(self, image, **params):
        return F.normalize_multispectral(image, self.mean, self.std)
